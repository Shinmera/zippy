...
(defun decode-msdos-timestamp (date time)
  (let ((yy (ldb (byte 6 9) date))
        (mm (ldb (byte 3 5) date))
        (dd (ldb (byte 4 0) date))
        (h (ldb (byte 4 11) time))
        (m (ldb (byte 5 5) time))
        (s (ldb (byte 4 0) time)))
    (flet ((clamp (l x h)
             (min h (max l x))))
      (encode-universal-time (clamp 0 (* 2 s) 59) (clamp 0 m 59) (clamp 0 h 23) (clamp 1 dd 31) (clamp 1 mm 12) (+ 1980 yy)))))

(defun encode-msdos-timestamp (timestamp)
  (multiple-value-bind (s m h dd mm yy) (decode-universal-time timestamp NIL)
    (let ((date 0)
          (time 0))
      (setf (ldb (byte 6 9) date) (- yy 1980))
      (setf (ldb (byte 3 5) date) mm)
      (setf (ldb (byte 4 0) date) dd)
      (setf (ldb (byte 4 11) time) h)
      (setf (ldb (byte 5 5) time) m)
      (setf (ldb (byte 4 0) time) (floor s 2))
      (values date time))))
